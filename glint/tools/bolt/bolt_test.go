...
func TestDelete(t *testing.T) {
	...
	db.View(func(tx *bbolt.Tx) error {
		...
		sobj := bobj.Bucket([]byte("2000.000"))
		assert.Nil(t, sobj)
		return nil
	})
}
...
func TestExists(t *testing.T) {
	...
	ok, err = Exists(db, "alpha", "1000.000")
	assert.True(t, ok)
	assert.NoError(t, err)
...
func TestGet(t *testing.T) {
	...
	// success
	pairs, err := Get(db, "alpha", "1000.000")
	assert.Equal(t, test.MockData["alpha"]["1000.000"], pairs)
	assert.NoError(t, err)
}
...
func TestList(t *testing.T) {
	...
	assert.Equal(t, []string{"1000.000", "1100.000"}, subbs)
	assert.NoError(t, err)
}
